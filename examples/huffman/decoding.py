...
from huffman import huff_tree, huff_code, write_dot, print_code, make_tree, iterdecode


def main():
    p = OptionParser("usage: %prog [options] [FILE]")
    p.add_option("-p", "--print", action="store_true", help="print Huffman code")
    p.add_option(
        "-t", "--tree", action="store_true", help="store the tree as a .dot file"
    )
