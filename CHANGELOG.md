...
*1.9.0* (2021-04-03):
  ...
  * add shift operations (`<<`, `>>`, `<<=`, `>>=`), see [##117](bitarray#117)
...
*1.8.2* (2021-03-31):
  ...
  * fix crash caused by unsupported types in binary operations, [##116](bitarray#116)
...
*1.7.1* (2021-03-12):
  ...
  * fix issue [##114](bitarray#114), raise TypeError when incorrect index is used during
...
*1.6.3* (2021-01-20):
  ...
  * add missing .h files to sdist tarball, [##113](bitarray#113)


*1.6.2* (2021-01-20):

  * use `Py_SET_TYPE()` and `Py_SET_SIZE()` for Python 3.10, [##109](bitarray#109)
  * add official Python 3.10 support
  * fix slice assignment to same object,
    e.g. `a[2::] = a` or `a[::-1] = a`, [##112](bitarray#112)
  * add bitarray.h, [##110](bitarray#110)
...
*1.6.0* (2020-10-17):
  ...
  * add `decodetree` object, for speeding up consecutive calls
    ...
    with large prefix codes, see [##103](bitarray#103)
  ...
  * update license file, [##104](bitarray#104)


*1.5.3* (2020-08-24):

  * add optional index parameter to `.index()` to invert single bit
  * fix `sys.getsizeof(bitarray)` by adding `.__sizeof__()`, see issue [##100](bitarray#100)


*1.5.2* (2020-08-16):

  * add PyType_Ready usage, issue [##66](bitarray#66)
  * speedup search() for bitarrays with length 1 in sparse bitarrays,
    see issue [##67](bitarray#67)
  * add tests
...
*1.2.2* (2020-05-18):
  ...
  * `util.ba2hex()` now always return a string object (instead of bytes
    object for Python 3), see issue [##94](bitarray#94)
  * `util.hex2ba` allows a unicode object as input on Python 2
  * Determine 64-bitness of interpreter in a cross-platform fashion [##91](bitarray#91),
...
*1.1.0* (2019-11-07):
  ...
  * fix issue [##74](bitarray#74), bitarray is hashable on Python 2
  * fix issue [##68](bitarray#68), `unittest.TestCase.assert_` deprecated
...
*0.9.2* (2019-04-29):
  ...
  * fix to compile on Windows with VS 2015, issue [##72](bitarray#72)


*0.9.1* (2019-04-28):

  * fix types to actually be types, [##29](bitarray#29)
...
*0.9.0* (2019-04-22):
  ...
  * more efficient decode and iterdecode by using C-level binary tree
    instead of a python one, [##54](bitarray#54)
  * added buffer protocol support for Python 3, [##55](bitarray#55)
  * fixed invalid pointer exceptions in pypy, [##47](bitarray#47)
...
*0.8.2* (2018-05-30):
  ...
  * fix description of `fill()`, [##52](bitarray#52)
  * handle extending self correctly, [##28](bitarray#28)
  * copy_n: fast copy with memmove fixed, [##43](bitarray#43)
  * minor clarity/wording changes to README, [##23](bitarray#23)


*0.8.1* (2013-03-30):

  * fix issue [##10](bitarray#10), i.e. `int(bitarray())` segfault
